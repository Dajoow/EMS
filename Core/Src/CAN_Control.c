...
// 查表法计算crc
uint16_t
CRC16_modbus (unsigned char *pucFrame, uint16_t usLen)
{
...
// 返回BCMU_Mail_t结构体
BCMU_Mail_t *
ReturnBCMU_Mail (CANRxMsg_t *CANRxMsg)
{
    uint32_t StdId;
...
// 软件定时器回调
void
CANTimerCallback01 (void const *argument)
{
    if (1) {}
}
 ...
 ***********************************************/
int8_t
CANFrameSend (uint8_t cmd, uint8_t *pdata, uint8_t *msg, uint16_t len)
{
    uint16_t index    = 0;
    uint16_t crc_temp = 0;
    uint16_t count    = 0;
    ...
    for (uint16_t i = 0; i < len; i++) { msg[index++] = pdata[i]; }
    ...
    for (uint16_t i = 0; i < index / CAN_Snd_DLC + 1; i++) {
        ...
        count = 0;
        while (HAL_FDCAN_GetTxFifoFreeLevel (&Myfdcan) < 3 && count < UINT16_MAX) { count++; } // wait the FiFo free
            ...
            switch (state) {
                case CHARGE_CTRL_SW_CLOSE:
                    ...
                    while (waitting) {
                        ...
                        if (waitting > 300) { break; }
                        osDelay (100);
                    }
                    // if not timeout
                    if (waitting == 0) { station_charge_ctrl.data.charge_ctrl = CHARGE_CTRL_ACK; }
                    break;
...
// 成功接收后处理函数
void
CAN_DataHandle (uint8_t Queue_NUM_t, uint8_t cmd, void *data, uint16_t len)
{
    ...
    memset (Client_errors[Queue_NUM_POLL], 0, MAX_ERROR * sizeof (error_info_t));

    void *ptr_err_start  = (void *)((uint8_t *)data + CLIENT_SD_ERR_OFT + sizeof (error_info_t));
    ...
    memset (bmu_offline[Queue_NUM_POLL], 0, sizeof (bmu_offline[Queue_NUM_POLL]));
                ...
                while (xReturn == pdTRUE) {
                    switch (frame_status[Queue_NUM]) {
                        ...
                        case frame_datastatus: // 接收数据段
                            BCMU[Queue_NUM].Data_buf[index[Queue_NUM]] = data;
                            index[Queue_NUM]++;
                            if (index[Queue_NUM] == len[Queue_NUM] + 5) { frame_status[Queue_NUM] = frame_crc1status; }
                            break;
                        ...
                        case handle: // 校验CRC 低8位
                            frame_status[Queue_NUM] = frame_head1status;
                            // 成功接收后处理函数
                            CAN_DataHandle (Queue_NUM, comand[Queue_NUM], (void *)(FDCAN_REV_Buff[Queue_NUM] + 5),
                                            len[Queue_NUM]);
                            // 有用包数+1
