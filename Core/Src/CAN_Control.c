...
#if defined(__CC_ARM)
uint8_t FDCAN_REV_Buff[cluster_num][FDCAN_REV_BuffSize]
    __attribute__((at(CANRevBuffStartAdd))); // FDCAN接收缓存区  8k*20
uint8_t FDCAN_SND_Buff[FDCAN_SND_BuffSize]
    __attribute__((at(CANSndBuffStartAdd))); // FDCAN发送缓存区  32k
#elif defined(__GNUC__)
uint8_t FDCAN_REV_Buff[cluster_num][FDCAN_REV_BuffSize]
    __attribute__((section(".CANRevBuff"))); // FDCAN接收缓存区  8k*20
uint8_t FDCAN_SND_Buff[FDCAN_SND_BuffSize]
    __attribute__((section(".CANSndBuff"))); // FDCAN发送缓存区  32k
#endif
BCMU_Mail_t BCMU[cluster_num];
uint8_t BCMU_ID[cluster_num][2] = {
    BCMU1_ID >> 8,  BCMU1_ID & 0x00ff,  BCMU2_ID >> 8,  BCMU2_ID & 0x00ff,
    BCMU3_ID >> 8,  BCMU3_ID & 0x00ff,  BCMU4_ID >> 8,  BCMU4_ID & 0x00ff,
    BCMU5_ID >> 8,  BCMU5_ID & 0x00ff,  BCMU6_ID >> 8,  BCMU6_ID & 0x00ff,
    BCMU7_ID >> 8,  BCMU7_ID & 0x00ff,  BCMU8_ID >> 8,  BCMU8_ID & 0x00ff,
    BCMU9_ID >> 8,  BCMU9_ID & 0x00ff,  BCMU10_ID >> 8, BCMU10_ID & 0x00ff,
    BCMU11_ID >> 8, BCMU11_ID & 0x00ff, BCMU12_ID >> 8, BCMU12_ID & 0x00ff,
    BCMU13_ID >> 8, BCMU13_ID & 0x00ff, BCMU14_ID >> 8, BCMU14_ID & 0x00ff,
    BCMU15_ID >> 8, BCMU15_ID & 0x00ff, BCMU16_ID >> 8, BCMU16_ID & 0x00ff,
    BCMU17_ID >> 8, BCMU17_ID & 0x00ff, BCMU18_ID >> 8, BCMU18_ID & 0x00ff,
...
// 标准帧滤波器, 扩展帧滤波器设置, 开启新消息接收中断, 开启Bus-Off中断,
// 设置发送传输延时补偿等
void fdcan_config(void) {
  ...
  HAL_FDCAN_ConfigGlobalFilter(&Myfdcan, FDCAN_REJECT, FDCAN_REJECT,
  ...
  HAL_FDCAN_ConfigTxDelayCompensation(
      &Myfdcan, Myfdcan.Init.DataPrescaler * Myfdcan.Init.DataTimeSeg1, 0);
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan,
  ...
  if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != 0) {
    if (hfdcan->Instance == FDCAN1) {
      ...
      //&(RxMsg.RxHeader), RxMsg.payload); 			for(uint8_t i=0; i<CAN_Rev_DLC; i++)
      //				xQueueSendFromISR(CANQueueHandle,
      //&RxMsg.payload[i], NULL);
    }
    if (hfdcan->Instance == FDCAN2) {
      HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &(RxMsg.RxHeader),
                             RxMsg.payload);
      BCMU_Mail_t *BCMUn = ReturnBCMU_Mail(&RxMsg); // 返回BCMU_Mail_t结构体
      if (BCMUn != NULL) {
        for (uint8_t i = 0; i < CAN_Rev_DLC; i++)
          xQueueSendFromISR(*(BCMUn->BCMUQueue), &RxMsg.payload[i],
                            NULL); // 进相应的消息队列
      }
    }
  }
...
void CAN_DataHandle(uint8_t Queue_NUM_t, uint8_t cmd, void *data,
  ...
  uint32_t error_data_num = *((uint32_t *)data + CLIENT_SD_ERR_OFT / sizeof(uint32_t));
  uint32_t error_data_len = error_data_num * sizeof (error_info_t);
  
  memset (Client_errors[Queue_NUM_POLL], 0,
          MAX_ERROR * sizeof (error_info_t));

  void *ptr_err_start
      = (void *)((uint8_t *)data + CLIENT_SD_ERR_OFT + sizeof (error_info_t));
  void *ptr_tail_start = (void *)((uint8_t *)ptr_err_start + error_data_len);

  memcpy((void *)&Client_Sd[Queue_NUM_POLL], data, CLIENT_SD_ERR_OFT + sizeof (error_info_t));
      ...
      if (xReturn == pdTRUE) {
        while (xReturn == pdTRUE) {
          ...
          case frame_crc2status: // 校验CRC 低8位
            ...
            // 校验CRC是否一致
            crc_temp =
                CRC16_modbus(BCMU[Queue_NUM].Data_buf, index[Queue_NUM] - 2);
          ...
          case handle: // 校验CRC 低8位
            frame_status[Queue_NUM] = frame_head1status;
            // 成功接收后处理函数
            CAN_DataHandle(Queue_NUM, comand[Queue_NUM],
                           (void *)(FDCAN_REV_Buff[Queue_NUM] + 5),
                           len[Queue_NUM]);
            // 有用包数+1
          ...
          xReturn = xQueueReceive(CANQueueHandle[Queue_NUM], &data,
                                  0); // 消息队列中还有数据，则继续出队
        }
...
void CAN_Poll(void const *argument) {
  /* 配置发送参数 */
  TxMsg.TxHeader.Identifier =
      CAN_ID; /* 设置接收帧消息的ID 11位帧ID范围0x000-0x7ff*/
  TxMsg.TxHeader.IdType = FDCAN_STANDARD_ID;     /* 标准ID */
  TxMsg.TxHeader.TxFrameType = FDCAN_DATA_FRAME; /* 数据帧 */
  TxMsg.TxHeader.DataLength = FDCAN_DLC_BYTES_64;
      /* 发送数据长度 */                                 // 现在固定为64
  ...
  TxMsg.TxHeader.MessageMarker =
      0; /* 用于复制到TX EVENT FIFO的消息Maker来识别消息状态，范围0到0xFF */
         //	osTimerStart(CANTimer01Handle, 1000);
         ////开启FREERTOS软件定时器once
  for (;;) {
    uint8_t ACK_ERROR_TIME = 0;
    for (Queue_NUM_POLL = 0; Queue_NUM_POLL < cluster_num;) {
      if (Queue_NUM_POLL < bsmuSetting.cu_num) {
        CANFrameSend(CMD_TRANS_START, (unsigned char *)&BCMU_ID[Queue_NUM_POLL],
                     FDCAN_SND_Buff, 2);
