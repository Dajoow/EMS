...
const ip_addr_t sntp_server[SNTP_MAX_SERVERS] = {
    ...
    {.addr = IPADDR4_INIT_BYTES (119, 28, 183, 184)},     // asia.pool.ntp.org
};

void sntp_set_rtc(uint32_t timestamp){
    RTC_TimeTypeDef sTime = { 0 };
    RTC_DateTypeDef sDate = { 0 };
    struct tm *utc_time = gmtime (&timestamp); // use UTC time
    ...
    sDate.Year = (utc_time->tm_year) + 1900 - 2000; // 0-99
    ...
    Debug_printf ("RTC time updated, timestamp:%lu", timestamp);
}
...
uint32_t rtc_get_timestamp(void){
    ...
    utc_time.tm_year = g_Date.Year + 100;    //RTC_Year rang 0-99,but tm_year since 1900
    utc_time.tm_mon = g_Date.Month - 1;      //RTC_Month rang 1-12,but tm_mon rang 0-11
    ...
    utc_time.tm_min = g_Time.Minutes;        //RTC_Minutes rang 0-59 and tm_min rang 0-59
