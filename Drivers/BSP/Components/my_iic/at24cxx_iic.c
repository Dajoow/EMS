#include "at24cxx_iic.h"


void AT24CXX_IIC_Init(void)
{
	GPIO_InitTypeDef GPIO_InitStruct;
	
	__HAL_RCC_GPIOD_CLK_ENABLE();   //使能GPIOD时钟
    
  	GPIO_InitStruct.Pin = AT24CXX_SCL_Pin|AT24CXX_SDA_Pin;
  	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  	GPIO_InitStruct.Pull = GPIO_NOPULL;
  	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
  	HAL_GPIO_Init(GPIOD, &GPIO_InitStruct);
    
	AT24CXX_IIC_SDA(1);
	AT24CXX_IIC_SCL(1);  
}


//产生IIC起始信号
void AT24CXX_IIC_Start(void)
{
	AT24CXX_I2C_SDA_OUT();     //sda线输出
	AT24CXX_IIC_SDA(1);	  	  
	AT24CXX_IIC_SCL(1);
	MyDelay_us(4);
 	AT24CXX_IIC_SDA(0);//START:when CLK is high,DATA change form high to low 
	MyDelay_us(4);
	AT24CXX_IIC_SCL(0);//钳住I2C总线，准备发送或接收数据 
}	  

//产生IIC停止信号
void AT24CXX_IIC_Stop(void)
{
	AT24CXX_I2C_SDA_OUT();//sda线输出
	AT24CXX_IIC_SCL(0);
	AT24CXX_IIC_SDA(0);//STOP:when CLK is high DATA change form low to high
 	MyDelay_us(4);
	AT24CXX_IIC_SCL(1); 
	AT24CXX_IIC_SDA(1);//发送I2C总线结束信号
	MyDelay_us(4);							   	
}

//等待应答信号到来
//返回值：1，接收应答失败
//        0，接收应答成功
uint8_t AT24CXX_IIC_Wait_Ack(void)
{
	uint8_t ucErrTime=0;
	AT24CXX_I2C_SDA_IN();      //SDA设置为输入  
	AT24CXX_IIC_SDA(1);MyDelay_us(1);	   
	AT24CXX_IIC_SCL(1);MyDelay_us(1);	 
	while(AT24CXX_I2C_READ_SDA)
	{
		ucErrTime++;
		if(ucErrTime>250)
		{
			AT24CXX_IIC_Stop();
			return 1;
		}
	}
	AT24CXX_IIC_SCL(0);//时钟输出0 	   
	return 0;  
} 

//产生ACK应答
void AT24CXX_IIC_Ack(void)
{
	AT24CXX_IIC_SCL(0);
	AT24CXX_I2C_SDA_OUT();
	AT24CXX_IIC_SDA(0);
	MyDelay_us(2);
	AT24CXX_IIC_SCL(1);
	MyDelay_us(2);
	AT24CXX_IIC_SCL(0);
}
//不产生ACK应答		    
void AT24CXX_IIC_NAck(void)
{
	AT24CXX_IIC_SCL(0);
	AT24CXX_I2C_SDA_OUT();
	AT24CXX_IIC_SDA(1);
	MyDelay_us(2);
	AT24CXX_IIC_SCL(1);
	MyDelay_us(2);
	AT24CXX_IIC_SCL(0);
}


//IIC发送一个字节
//返回从机有无应答
//1，有应答
//0，无应答			  
void AT24CXX_IIC_Send_Byte(uint8_t txd)
{                        
    uint8_t t;   
		AT24CXX_I2C_SDA_OUT(); 	    
    AT24CXX_IIC_SCL(0);//拉低时钟开始数据传输
    for(t=0;t<8;t++)
    {              
        AT24CXX_IIC_SDA((txd&0x80)>>7);
        txd<<=1; 	  
		MyDelay_us(2);   //这三个延时都是必须的
		AT24CXX_IIC_SCL(1);
		MyDelay_us(2); 
		AT24CXX_IIC_SCL(0);	
		MyDelay_us(2);
    }	 
} 

//读1个字节，ack=1时，发送ACK，ack=0，发送nACK   
uint8_t AT24CXX_IIC_Read_Byte(unsigned char ack)
{
	unsigned char i,receive=0;
	AT24CXX_I2C_SDA_IN();//SDA设置为输入
    for(i=0;i<8;i++ )
	{
        AT24CXX_IIC_SCL(0); 
        MyDelay_us(2);
				AT24CXX_IIC_SCL(1);
        receive<<=1;
        if(AT24CXX_I2C_READ_SDA)receive++;   
				MyDelay_us(1); 
    }					 
    if (!ack)
        AT24CXX_IIC_NAck();//发送nACK
    else
        AT24CXX_IIC_Ack(); //发送ACK   
    return receive;
}






