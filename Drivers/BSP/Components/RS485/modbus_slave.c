...
static int read_registers(agile_modbus_t *ctx, struct agile_modbus_slave_info *slave_info, const agile_modbus_slave_util_t *slave_util)
{
    ...
    for (int now_address = address, i = 0; now_address < address + nb; now_address++, i++) {
        ...
        now_address += map_len - 1; // start addr of next sliding window
        i += map_len - 1; // next map
    }
...
static int write_registers(agile_modbus_t *ctx, struct agile_modbus_slave_info *slave_info, const agile_modbus_slave_util_t *slave_util)
{
    ...
    for (int now_address = address, i = 0; now_address < address + nb; now_address++, i++) {
        ...
        if (slave_util->set) {
            ...
            int rc = slave_util->set(map, index, need_len, map_buf, sizeof(map_buf));
            if (rc != 0)
                return rc;
        }
...
static int mask_write_register(agile_modbus_t *ctx, struct agile_modbus_slave_info *slave_info, const agile_modbus_slave_util_t *slave_util)
{
    ...
    if (slave_util->set) {
        memset(map_buf, 0, sizeof(map_buf));
        if (slave_util->get) {
            slave_util->get(map, map_buf, address, 1);
        }
        ...
        int rc = slave_util->set(map, index, 1, map_buf, sizeof(map_buf));
        if (rc != 0)
            return rc;
    }
...
agile_modbus_slave_util_t* get_data_by_slave_addr(struct agile_modbus_slave_info *slave_info, const bsmu_modbus_data_t *data){
    ...
    for (int i = 0; i < nb; i++)
    {
        if(data[i].slave_addr == target_slave){
            return data[i].slave_util;
        }
    }
...
int bsmu_modbus_slave_callback(agile_modbus_t *ctx, struct agile_modbus_slave_info *slave_info, const void *data)
{
    int function = slave_info->sft->function;
    int ret = 0;
    agile_modbus_slave_util_t *slave_util = get_data_by_slave_addr(slave_info,(const bsmu_modbus_data_t *)data);
